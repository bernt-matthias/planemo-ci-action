...
if [ "$PLANEMO_COMBINE_OUTPUTS" == "true" ]; then
  ls -R artifacts/ 
  find -D exec artifacts/ -name tool_test_output.json -exec sh -c 'echo planemo merge_test_reports "$@" upload/tool_test_output.json' sh {} +
  find -D exec artifacts/ -name tool_test_output.json -exec sh -c 'planemo merge_test_reports "$@" upload/tool_test_output.json2' sh {} +
  cat upload/tool_test_output.json2
