...
if [ "$MODE" == "test" ]; then
  ...
  while read -r -a TOOL_GROUP; do
    ...
    PIP_QUIET=1 planemo test --conda_channels OpenMS,conda-forge,bioconda,defaults "${PLANEMO_OPTIONS[@]}" "${PLANEMO_TEST_OPTIONS[@]}" --test_output_json "$json" "${TOOL_GROUP[@]}" || true
