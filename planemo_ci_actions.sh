...
if [ "$PLANEMO_COMBINE_OUTPUTS" == "true" ]; then
  ...
  [ "$PLANEMO_HTML_REPORT" == "true" ] && planemo test_reports upload/tool_test_output.json --test_output upload/test_output.html
  [ "$PLANEMO_MD_REPORT" == "true" ] && planemo test_reports upload/tool_test_output.json --test_output_markdown upload/test_output.md
else
